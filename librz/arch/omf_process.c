...
static OMF_components *get_component_by_ti(rz_bin_omf166_obj *omf_obj, ut16 ti) {
	bool found = false;
...
static inline OMF_type *OMF_TYPE_TI(rz_bin_omf166_obj *omf_obj, ut16 ti) {
	bool found = false;
...
static bool omf_try_create_var_global(
	...
	rz_pvector_foreach (v, it) {
		...
		RZ_LOG_DEBUG("\ncreate_var_global - data base: [0x%08x] Ofs16:[0x%04x] Rep8:[0x%02x] [%10s V%s bpos: %d] TI:[0x%04x] (%d)`%s`\n",
			p->base, p->offset, p->REP8,
			name_of_rep8(p->REP8), p->V ? "-" : "+", p->bpos,
			p->ti, p->n, p->name2);
		RzAnalysisVarGlobal *exist_var = rz_analysis_var_global_get_byname(analysis, p->name2);
		if (exist_var) {
			RZ_LOG_DEBUG("var exist [0x%08x] Ofs16:[0x%04x] Rep8:[0x%02x] [%10s V%s bpos: %d] TI:[0x%04x] (%d)`%s`\n",
				p->base, p->offset, p->REP8,
				name_of_rep8(p->REP8),
				p->V ? "-" : "+", p->bpos,
				p->ti, p->n, p->name2);
...
static bool types_cb(void *user, const ut64 k, const void *v) {
	...
	if (type->descr_type == STRUCT_UNION_DESCRIPTOR) {
		...
		OMF_components *components = get_component_by_ti(omf_obj, type->descriptor.struct_union.member_ti);
		if (components) {
			for (ut16 i = 0; i < components->count; i++) {
				OMF_component *component = (OMF_component *)components->comp + i;
				if (!component)
		...
		RzBaseType *btype = rz_type_db_get_base_type(omf_obj->typedb, type->descriptor.struct_union.tagname);
		if (btype) {
	...
	if (type->descr_type == FUNCTION_DESCRIPTOR) {
		...
		if (!(cal->args = rz_pvector_new((RzPVectorFree)rz_type_callable_arg_free))) {
			return false;
		}
 ...
 */
RZ_API void rz_analysis_omf166_integrate_functions(RzAnalysis *analysis, RzFlag *flags) {
...
RZ_API bool rz_core_bin_apply_omf_debug(RzCore *core, RzBinFile *binfile) {
	...
	RzPVector *vs = omf_obj->symbols_vec;
	void **vit;
