...
static void c166_op_rn_rm(RzAnalysis *analysis, RzAnalysisOp *op, ut8 nm, ut32 type, bool byte) {
	// printf("c166_op_rn_rm op->size: %d\n", op->size);
...
static void c166_op_set_type(RZ_NONNULL C166_Inst *instr, RzAnalysis *analysis, RzAnalysisOp *op, const ut8 *buf) {
	ut8 operand1 = get_operand(instr, 1);
	switch (instr->id) {
	...
	case C166_JMPA_cc_caddr:
		op->type = RZ_ANALYSIS_OP_TYPE_CJMP;
		op->cond = c166_cc_to_cond((operand1 >> 4) & 0xF);
		c166_set_jump_target_from_caddr(op, rz_read_at_le16(buf, 2));
		op->fail = instr->addr + op->size;
...
static char *get_reg_profile(RzAnalysis *analysis) {
	const char *p =
		...
		"seg	DPP0	.16	65024	0\n" // FE00; CPU Data Page Pointer 0 Register (4 bits)
		"seg	DPP1	.16	65026	0\n" // FE02; CPU Data Page Pointer 1 Register (4 bits)
		"seg	DPP2	.16	65028	0\n" // FE04; CPU Data Page Pointer 2 Register (4 bits)
		"seg	DPP3	.16	65030	0\n" // FE06; CPU Data Page Pointer 3 Register (4 bits)
		"seg	CSP		.16	65032	0\n" // FE08; CPU Code Segment Pointer Register (2 bits, read only)
		...
		"gpr	r_FFDA	.16	65498	0\n" // FFDA; RESERVED
		"gpr	r_FFDC	.16	65500	0\n" // FFDC; RESERVED
		"gpr	r_FFDE	.16	65501	0\n"; // FFDE; RESERVED
	return strdup(p);
}
