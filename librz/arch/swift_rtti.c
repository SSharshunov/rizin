...
static void rz_add_metaclass_info(HtUP *swift_metaclass_info, RzList *fields, char *class_name) {
	RzListIter *iter;
...
RZ_API void rz_analysis_swift_rtti(RzAnalysis *analysis) {
	HtUP *swift_metaclass_info = ht_up_new(HT_STR_DUP, free);
	...
	rz_pvector_foreach (classes, iter_class) {
		...
		if (fields) {
			rz_add_metaclass_info(swift_metaclass_info, fields, class->name);
		}
	}
