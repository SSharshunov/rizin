...
static void rz_bin_process_metaclass(RzBinObject *o, RzBinSymbol *symbol) {
	...
	rz_bin_object_add_field(o, classname + sizeof("type metadata for ") - 1, symbol->dname, symbol->paddr, symbol->vaddr);
}

// This function is used to process Swift methods.
static void rz_bin_process_swift_class_method(RzBinObject *o, RzBinSymbol *symbol) {
...
RZ_IPI void rz_bin_process_swift(RzBinObject *o, RzBinSymbol *symbol) {
	...
	rz_bin_process_metaclass(o, symbol);
	rz_bin_process_swift_class_method(o, symbol);
}
